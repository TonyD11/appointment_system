...
func (h *Handler) GetAppointments(w http.ResponseWriter, r *http.Request) {
	...
	opts := options.Find().SetSort(bson.D{{Key: "appointmentDate", Value: 1}})
...
func (h *Handler) CreateAppointment(w http.ResponseWriter, r *http.Request) {
	// Read body including appointmentDate as string
	var body struct {
		PatientName     string `json:"patientName"`
		DoctorID        string `json:"doctorId"`
		ContactNumber   string `json:"contactNumber"`
		AppointmentDate string `json:"appointmentDate"` // comes as string from frontend
	}
	json.NewDecoder(r.Body).Decode(&body)

	if body.PatientName == "" || body.DoctorID == "" || body.ContactNumber == "" || body.AppointmentDate == "" {
		sendJSON(w, 400, map[string]string{"error": "Please fill all fields including date and time"})
		return
	}

	// Parse the date string into time.Time
	appointmentDate, err := time.Parse("2006-01-02T15:04", body.AppointmentDate)
	if err != nil {
		sendJSON(w, 400, map[string]string{"error": "Invalid date format"})
		return
	}

	// Check date is in the future
	if appointmentDate.Before(time.Now()) {
		sendJSON(w, 400, map[string]string{"error": "Appointment date must be in the future"})
		return
	}
